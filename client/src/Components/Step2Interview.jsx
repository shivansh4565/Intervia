...
function Step2Interview({ interviewData, onFinish }) {
  ...
  const userName = interviewData?.userName ?? interviewData?.name ?? "There"
  ...
  useEffect(() => {
    ...
    const female = voices.find(v => v.name.toLowerCase().includes("zira"));
    const male = voices.find(v => v.name.toLowerCase().includes("david"));
  ...
  useEffect(() => {
    ...
    const run = async () => {
      if (isIntroPhase) {
        ...
        await speakText(typeof currentQuestion === "string"
          ? currentQuestion
          : currentQuestion?.question
        );
      }
  ...
  return (
    <div className='min-h-screen flex items-center justify-center p-4'>
      <div className="w-full max-w-6xl bg-white rounded-3xl shadow-2xl flex">
        
        <div className="w-1/3 p-6">
          <video className='w-full rounded-xl' muted src={voiceGender === "male" ? maleVideo : femaleVideo} ref={videoRef}/>
          {subtitle && <p className="mt-4 text-center">{subtitle}</p>}
        ...
        <div className="flex-1 p-6">
          ...
          <p className="mb-2">Question {currentIndex + 1} / {questions.length}</p>
